...
import { openJobsStore, persistExtractedJob } from "../lib/extract-job-logic.ts";
 ...
 *
 * Turns one job snapshot's posting text into typed fields on that same
 * snapshot. Thin and typed, per the P02 rule (README "Extending the runner"
 * §4) and the iter-003 decision (model tools take IDs only): the model never
 * passes a URL or raw posting text here, only the snapshot's `jobId` and
 * `revision` (`captures.ts`'s extraction turn gives it those, and delivers
 * the posting text as user-turn data in the same prompt — never in a system
 * prompt; mvp-spec §7.2, hard-problems.md #3) and the structured fields it
 * drafted from that text. Durable (`defineWorkflowTool`).
 *
