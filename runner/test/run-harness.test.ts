import type { Client, InputRequest, MessageResponse, MessageResult, MessageStreamEvent } from "eve/client";
import { describe, expect, it } from "vitest";
import { ManualClock } from "../lib/clock.ts";
import { createRunnerContext } from "../server/context.ts";
import type { EveGateway } from "../server/eve-gateway.ts";
import { PROVIDER_LIMIT_REASON, runTurn, withRun, type RunBodyResult, type TurnResult } from "../server/run-harness.ts";
...
function failed(code: string, message: string, details?: Record<string, string>): MessageStreamEvent {
  return { type: "turn.failed", data: { code, message, ...(details ? { details } : {}), sequence: 9, turnId: "t1" }, meta: META };
}

function messageResult(overrides: Partial<MessageResult> = {}): MessageResult {
  return { data: undefined, message: "done", events: [], inputRequests: [], sessionId: "s1", status: "completed", ...overrides };
}
...
/** A fake EveGateway whose client.sessions.create is scripted per call. cancel() is counted so "parked"/"timeout" cancellation can be asserted. */
function fakeEve(script: (call: FakeCall, signal: AbortSignal) => Promise<MessageResult> | "hang"): { eve: EveGateway; calls: FakeCall[]; cancelCount: () => number } {
  ...
  const create = async (input: { message: string; signal?: AbortSignal }) => {
    ...
    const result: MessageResult = outcome === "hang" ? await new Promise<MessageResult>((_resolve, reject) => signal.addEventListener("abort", () => reject(new Error("aborted")))) : await outcome;
    const response = {
      sessionId: result.sessionId,
      result: async () => result,
      cancel: async () => {
        cancelCount += 1;
        return { status: "accepted" as const, sessionId: result.sessionId };
      },
    } as unknown as MessageResponse;
    return { response, session: undefined as unknown };
  };
...
describe("run-harness.ts: runTurn", () => {
  it("an ok turn sums tokens and takes the model id from the last step.started", async () => {
    const { eve } = fakeEve(async () => messageResult({ events: [started("gpt-5.6-luna"), completed({ inputTokens: 100, outputTokens: 20 })] }));
  ...
  it("sums usage across more than one step", async () => {
    const { eve } = fakeEve(async () =>
      messageResult({ events: [started("m1", 0), completed({ inputTokens: 10, outputTokens: 2 }, 0), started("m2", 1), completed({ inputTokens: 5, outputTokens: 1 }, 1)] }),
    );
    const { ctx } = await contextWith(eve);
    const result = await runTurn(ctx, { message: "go" });
  ...
  it("an ordinary failure fails the turn without pausing the budget (negative case)", async () => {
    const { eve } = fakeEve(async () => messageResult({ status: "failed", events: [failed("MODEL_CALL_FAILED", "The model declined to answer.")] }));
  ...
  it("primary signal: details.semanticErrorId gateway-rate-limited pauses the budget with reason 'provider limit'", async () => {
    const { eve } = fakeEve(async () => messageResult({ status: "failed", events: [failed("MODEL_CALL_FAILED", "AI Gateway rate-limited the request.", { semanticErrorId: "gateway-rate-limited" })] }));
    const { ctx, workspace, clock } = await contextWith(eve);
    const result = await runTurn(ctx, { message: "go" });
    expect(result.status).toBe("failed");
  ...
  it("primary signal: details.semanticErrorId gateway-free-tier-rate-limited also pauses", async () => {
    const { eve } = fakeEve(async () => messageResult({ status: "failed", events: [failed("MODEL_CALL_FAILED", "Free tier requests on this model are rate-limited.", { semanticErrorId: "gateway-free-tier-rate-limited" })] }));
    const { ctx, workspace, clock } = await contextWith(eve);
    const result = await runTurn(ctx, { message: "go" });
    expect(result.providerLimit).toBe(true);
  ...
  it("a semanticErrorId for an unrelated gateway rule does not count as a provider limit", async () => {
    const { eve } = fakeEve(async () => messageResult({ status: "failed", events: [failed("MODEL_CALL_FAILED", "The requested model is not available.", { semanticErrorId: "model-not-found" })] }));
    const { ctx, workspace, clock } = await contextWith(eve);
    const result = await runTurn(ctx, { message: "go" });
    expect(result.providerLimit).toBeFalsy();
    expect((await getBudgetState(workspace, clock)).paused).toBe(false);
  ...
  it("fallback signal (no semanticErrorId): a bare 429 in the message pauses the budget", async () => {
    const { eve } = fakeEve(async () => messageResult({ status: "failed", events: [failed("UPSTREAM_ERROR", "HTTP 429 Too Many Requests from the provider.")] }));
    const { ctx, workspace, clock } = await contextWith(eve);
    const result = await runTurn(ctx, { message: "go" });
    expect(result.providerLimit).toBe(true);
    expect((await getBudgetState(workspace, clock)).paused).toBe(true);
  ...
  it("fallback signal (no semanticErrorId): 'rate limit' wording in the code pauses the budget", async () => {
    const { eve } = fakeEve(async () => messageResult({ status: "failed", events: [failed("rate_limited", "Too many requests right now.")] }));
    const { ctx, workspace, clock } = await contextWith(eve);
    const result = await runTurn(ctx, { message: "go" });
    expect(result.providerLimit).toBe(true);
    expect((await getBudgetState(workspace, clock)).paused).toBe(true);
  ...
  it("negative case: no semanticErrorId and no 429/rate-limit wording never pauses", async () => {
    const { eve } = fakeEve(async () => messageResult({ status: "failed", events: [failed("VALIDATION_ERROR", "Invalid request: missing field 'foo'.")] }));
    const { ctx, workspace, clock } = await contextWith(eve);
    const result = await runTurn(ctx, { message: "go" });
    expect(result.providerLimit).toBeFalsy();
    expect(result.status).toBe("failed");
  ...
  it("a turn with pending input requests parks and cancels the turn", async () => {
    const inputRequests = [{ kind: "text" } as unknown as InputRequest];
    const { eve, cancelCount } = fakeEve(async () => messageResult({ status: "waiting", inputRequests }));
    const { ctx } = await contextWith(eve);
    const result = await runTurn(ctx, { message: "go" });
  ...
  it("times out and cancels when the response has already resolved but the caller's abort still needs a response handle", async () => {
...
describe("run-harness.ts: withRun — crash safety and outcome (decision 3)", () => {
...
describe("run-harness.ts: simulated 429 storm end to end", () => {
  it("pauses on the first provider-limit failure, records it, never retries, and refuses later runs until Resume", async () => {
    const { eve, calls } = fakeEve(async () => messageResult({ status: "failed", events: [failed("MODEL_CALL_FAILED", "rate limited", { semanticErrorId: "gateway-rate-limited" })] }));
    const { ctx, workspace, clock } = await contextWith(eve);
