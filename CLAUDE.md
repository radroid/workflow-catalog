...
The catalog dev server is `pnpm --filter catalog dev` (Next.js) once P00 lands. The runner is `eve dev` / `eve start` inside `runner/` (mode decided by the P02 spike; see `docs/spec/research/eve-runtime.md`). The bridge listens on `127.0.0.1:4310`, loopback only.
