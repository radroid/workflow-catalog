...
**F2 Template page and package versions (P9).** One page per workflow template: what it does, what it needs (sources, connections, browser permissions), the current package version with changelog, and a download link to the GitHub release asset. Accept: version and changelog rendered from `workflow.json`; the checksum is the SHA-256 of the release tarball, published by the release workflow beside the asset (a file inside the tarball cannot hold the tarball's own hash); the tarball URL points at a release, not at Vercel storage.
...
career-profile.json               claims[], sources{}, presentation[], preferences, boundaries, approval{version, at}, revisions[]
...
- **Models** in `runner/agent/agent.ts` via `defineAgent({ model })`: `chatgpt()` (default, bills the person's ChatGPT subscription through the Codex backend; when the Codex CLI is on `PATH` it owns the credentials via `codex login`, otherwise sign-in is only through `/login` inside the `eve dev` TUI; there is no `eve login`), `openai()` with `OPENAI_API_KEY`, `anthropic()` with `ANTHROPIC_API_KEY`, or a gateway string. eve's default slug `gpt-5.6-luna-fast` is **rejected** by the Codex backend for a ChatGPT account (spike, 2026-09-22), so the runner stores an explicit slug in settings (`gpt-5.6-luna` worked) and `doctor` checks it. `eve deploy` refuses `chatgpt()`; the runner never deploys.
- **Two run modes, one spike (P02 decides on day one):** (A) `eve build && eve start --host 127.0.0.1` fires cron schedules and needs `httpBasic()`/custom auth (`localDev()` is inert there); whether `chatgpt()` works under `eve start` is **undocumented**. (B) `eve dev --no-ui --host 127.0.0.1 --port 2000` brokers `/login` credentials and authenticates with `localDev()`, but never fires cron. If (A) fails the `chatgpt()` test, the runner runs (B) and our bridge server triggers schedules on its own clock via `POST /eve/v1/dev/schedules/<id>` or `POST /eve/v1/session`. Either way the person sees one command: `npm run runner`. **Decided 2026-09-22: mode A** ([eve-spike.md](research/eve-spike.md)). `chatgpt()` completed turns under `eve start` with Codex-owned credentials, `httpBasic()` failed closed, and a `* * * * *` schedule fired and completed on its own. Conditions: an explicit model slug the account accepts, `codex` on the runner's `PATH`, and never alternating modes on one `.eve/` directory. Mode B passed the same test and stays the fallback.
...
- **Privacy defaults.** The installer sets `EVE_TELEMETRY_DISABLED=1` (CLI telemetry is on by default) and `EVE_TRACES_CONTENT=off` (local traces store prompt and response content by default under `.eve/traces/`). Even so, a failed model call prints the full request body to the server log, and `.eve/.workflow-data` stores message content unencrypted (spike): runner logs and `.eve/` are personal data, never shared, and removed by uninstall.
