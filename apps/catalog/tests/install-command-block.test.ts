...
describe("install command blocks copy exactly their commands (P09.1 revision round, UI critic U2)", () => {
  it("<pre className=\"command-block\"> contains nothing but the commands.map(...) call — no separator/joiner between commands", () => {
    ...
    expect(commandBlockSource).toMatch(
      /<pre className="command-block">\s*\{commands\.map\(\(command, index\) => \(\s*<code className="command-line" key=\{index\}>\s*\{command\}\s*<\/code>\s*\)\)\}\s*<\/pre>/,
    );
  });
  ...
  it("never concatenates a prompt string (\"$ \", \"> \", or similar) onto a command inside CommandBlock", () => {
    ...
    // whole-file text match.
    expect(commandBlockSource).not.toMatch(/["'`]\$ ["'`]/);
    expect(commandBlockSource).not.toMatch(/["'`]>\s?["'`]\s*\+\s*command/);
    expect(commandBlockSource).not.toMatch(/\{command\}\s*\+/); // "{command} + " would mean something is appended after it
    expect(commandBlockSource).not.toMatch(/\+\s*\{?command\}?\s*\}/); // catches `{"$ " + command}`-style interpolation too
  });
