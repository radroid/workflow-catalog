...
- A superlative, role, or scope claim in `text` gets a question, no exceptions — ask what makes it true rather than assuming the source's own phrasing is airtight. Always-ask words: `led`, `founded`, `the only`, `fastest`, `maintainer`, `used by`. A claim whose `text` contains one of them as a whole word, in any letter case, gets a question; the words are examples, and every other superlative, role, or scope claim gets one too.
